...
extern Unsigned8 g_Class20_week_name1[8],g_Class20_week_name2[8];
extern Unsigned8 g_Class20_season_name1[8],g_Class20_season_name2[8];
	...
	switch(nameplate_id)
	{
		...
		case EM_NAME_PLATE_YEAR_OF_MANUFACT:
			...
			*p_data++ = (Unsigned8)((u16>>8) &0xFF);
			result_len = 2;
			break;
		case EM_NAME_PLATE_METER_CONSTANT:
			result_len = 4;
			memcpy((Unsigned8 *)p_data,"1600", result_len);
		break;
		case EM_NAME_PLATE_VOLAGE_RATING:
			result_len = 5;
			memcpy((Unsigned8 *)p_data,"230 V", result_len);
		break;
...
Integer32 R_USER_GetEMData(Unsigned16 em_data)
{
	...
	switch(em_data)
	{
		...
		/*Cumulative power on duration in minutes*/
			...
			result=result/60;
			break;
		case EM_CUCUMLARIVE_POWER_OFF_DURATION:
		/*Cumulative power on duration in minutes*/
			result=getCumPowerOffDuration();
			result=result/60;
			break;
		case EM_TEMPERATURE:
				result=InsTemp;
		break;
		case EM_POWERFAIL_COUNT:
			result=InsSave.PFailCount;
		break;
		...
		case EM_POWER_FACTOR:
			...
			if((Ins.LeadLag==1)&&(Ins.AppPower>0))
		...
		case EM_BLOCK_AVG_CURENT:
			...
			break;
		case EM_BLOCK_AVG_PHCURENT:
			result = InsSave.lsAvgPC;
			if(InsSave.PowerOn30>0)
				result = result/InsSave.PowerOn30;
			result = result/10;	
			
			break;
		case EM_BLOCK_AVG_NUCURENT:
			result = InsSave.lsAvgNC;
			if(InsSave.PowerOn30>0)
				result = result/InsSave.PowerOn30;
			result = result/10;	
...
)
{
	...
	buf->kWh_value = getByteValue(lsBuffer, 1, 2);;// kWh
	
	buf->kVAh_value = getByteValue(lsBuffer, 8, 2);;// kVAh

	lsAVal=getByteValue(lsBuffer, 7, 1);
	lsAVal<<=8;
	lsAVal+=getByteValue(lsBuffer, 6, 1);
	lsAVal<<=8;
	lsAVal+=getByteValue(lsBuffer, 5, 1);
	lsAVal<<=8;
	lsAVal+=getByteValue(lsBuffer, 4, 1);
	lsAVal<<=8;
	
	lsAVal+=getByteValue(lsBuffer, 3, 1);
	
	

	
	
	buf->AvgTem=lsAVal&0x3FF;
	lsAVal>>=10;
	buf->AvgI_value=lsAVal&0x3FF;
	lsAVal>>=10;
	buf->NuC_value=lsAVal&0x3FF;
	lsAVal>>=10;
	buf->phC_value=lsAVal&0x3FF;
	

	
	
	if(buf->kWh_value)
		buf->pf=(buf->kWh_value*100/buf->kVAh_value);
	else
		buf->pf=0;
		
	buf->kW_value=buf->kWh_value*ls_period;
	buf->kVA_value=buf->kVAh_value*ls_period;
...
)
{
	...
	buf->H1kW_value = getByteValue(lsBuffer, 12, 2)/10;// H1 kW MD
	buf->H1kVA_value = getByteValue(lsBuffer, 14, 2)/10;// H1 kVA MD
	buf->daykW_value = getByteValue(lsBuffer, 16, 2)/10;// day kW MD
	buf->daykVA_value = getByteValue(lsBuffer, 18, 2)/10;// day kVA MD
...
)
{
	...
	if((index+1)==g_Class07_Billing_EntriesInUse)
	{
		...
		buf->exp_active_energy=0;//InsSave.ExpCumkWh;
		...
		buf->exp_apparent_energy=0;//InsSave.ExpCumkVAh;
...
Unsigned16 R_OBIS_Class07_GetInstantEntry(class07_instant_entry_t *buf)
{
	...
	buf->ExpkWh_value = R_USER_GetEMData(EM_INTERNAL_DIAGNOSTICS);
	buf->ExpkVAh_value = R_USER_GetEMData(EM_PRE_BILL_DATE_READ);
	buf->poweroffdur=R_USER_GetEMData(EM_CUCUMLARIVE_POWER_OFF_DURATION);
	buf->temperature=R_USER_GetEMData(EM_TEMPERATURE);
