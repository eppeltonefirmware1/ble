...
{
  uint8_t MaxZoneNo=0,zoneno,sorttedzone=0;
  uint8_t buff[10];
  read_eeprom((uint8_t *)&buff,ZONE_TIME+seasonNO*25+16,9);
  
  for(zoneno=1;zoneno<(buff[0]%9);zoneno++)
  {
	
  	for(MaxZoneNo=zoneno;MaxZoneNo<(buff[0]%9);MaxZoneNo++)
		if(buff[zoneno]>buff[MaxZoneNo])
		{
			sorttedzone=buff[MaxZoneNo];
			buff[MaxZoneNo]=buff[zoneno];
			buff[zoneno]=sorttedzone;
		}
	
  }
  MaxZoneNo=0;
  for(zoneno=0;zoneno<=(buff[0]%9);zoneno++)
  {
	if(MaxZoneNo<buff[zoneno])  
		MaxZoneNo++;
  }
...
{   
  ...
  addr =GetTODLatestBillLoc(eventno)+ZONE_LEN*zoneno;
  return addr; 
}