...
#if (defined(HARMONICS_CAL_ENABLE) && (HARMONICS_CAL_ENABLE == 1))
...
//#define CT_COMP_CURRENT_U_LEVEL 	(14500UL)
//#define CT_COMP_CURRENT_U_COMP_VAL 	(8)

//#define CT_COMP_CURRENT_L_LEVEL 	(1500UL)
//#define CT_COMP_CURRENT_L_COMP_VAL 	(-6)
void addComp(void);
...
void doMeasurement(uint8_t isProd)
{
	...
	else if(Ins.AppPower<STARTING_POWER_THRESHOLD_APP_U)
	{
		Ins.AppPower=0;
		Ins.PowerFactor=100;
	}	
		...
		else 
		{
			...
			if(Ins.EffectiveI>0)
			{
				...
				else
				{
					...
					if(isEarth(BAT_EARTH_TAMPER_THRESHOLD)==0)
					{
						Ins.EffectiveI=0;
						Ins.PhCurrent=0;
						Ins.NeuCurrent=0;
...
//uint8_t checkExitVolt1(uint8_t checkCurrent)
//{
//	uint8_t status=0;
//	doDeltaSigmaADC_NM(CH_U,NM_SHORT_SAMPLE);
//	DSADMR &=_F0F0_AD_DSADMR_POWER_OFF;  /* Stop conversion */
...
//			{
//				Ins.PhCurrent+=(rlCalib.I1OffSet/2);
