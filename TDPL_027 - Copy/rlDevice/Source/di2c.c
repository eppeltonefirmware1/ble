...
#define	TEMP_DEVICE_BUS_ADDRESS				0x92		/* temperature device bus address */
#define	EPR_BUS_SPEED						60000		/* I2C Bus speed (Hz) */
#define	EPR_DEVICE_SIZE						0x10000UL	/* EEPROM device size (total bytes) */
...
{
	uint8_t membuf[4]={0x00,0x00,0x00,0x00};
	uint8_t status=1;
	uint16_t deviceOveraddr=0;
	
	if(EPR_DEVICE_SIZE>32768)
	{
		deviceOveraddr=32768-4;
		write_eeprom(membuf,deviceOveraddr,4);
	}
	else
		write_eeprom(membuf,0,4);
	...
	membuf[2]=0;
	membuf[3]=0;
	read_eeprom(membuf,deviceOveraddr,4);
